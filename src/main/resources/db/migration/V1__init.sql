...
create table product (id integer not null, created_date timestamp not null, last_modified_date timestamp not null, name varchar(255) not null, price double not null check (price>=0), stock integer not null check (stock>=0), provider_id integer, primary key (id));
create table provider (id integer not null, created_date timestamp not null, last_modified_date timestamp not null, name varchar(100) not null, primary key (id));
...
alter table product add constraint FKfmt6vehie8ep9pq0nnvv9een7 foreign key (provider_id) references provider;
